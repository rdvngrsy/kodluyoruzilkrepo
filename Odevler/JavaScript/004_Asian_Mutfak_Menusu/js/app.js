...
let cate = Array.from(new Set(menu.map(item => item.category))) 

/* Filtre Butonlarının Olusturulması */
function createButton() { 
    let divDom = document.createElement("div")
    category.unshift("All")
    category.forEach(filter => {
        let buttonDom = document.createElement("button")
        buttonDom.innerHTML = filter
        buttonDom.classList.add("btn","btn-warning","ms-3")
    ...
    buttonContainer.appendChild(divDom)
}

/* Menü bölümünün oluşturulması */
...
/* seçili menu olmaması yada All seçilmesi durumunda listelenecek menuler */
const listAllFoods = () => { 
    let allFoods = ""
